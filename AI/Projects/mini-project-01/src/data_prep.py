...
from typing import Tuple  # <-- اضافه شد
...
def dataset_overview(df: pd.DataFrame) -> dict:
    """Return the basic structural information required by Phase 1."""
...
def print_overview(df: pd.DataFrame) -> None:
    """Pretty print the dataset overview (Phase 1 deliverable)."""
...
def get_descriptive_stats(df: pd.DataFrame) -> pd.DataFrame:
    """Return descriptive statistics for all features except target."""
...
def clean_data(
    ...
    """
    Phase 2 data-quality step.
    - Handles missing values (mean imputation for features, drops rows for target)
    - Drops exact duplicate rows (common in this dataset)
    """
    df = df.copy()
...
def stratified_split(
    ...
    """
    Stratified train/test split, required because of class imbalance.
